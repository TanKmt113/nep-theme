...
@php
  // Nội dung biên tập (port từ NEP_DATA.about). Có thể chuyển sang ACF sau.
  $hero = 'https://images.unsplash.com/photo-1524758631624-e2822e304c36?w=1800&q=80';
  $lead = 'NẾP ra đời năm 2014 từ một xưởng may rèm nhỏ ở Sài Gòn, với niềm tin rằng một tấm rèm đẹp có thể thay đổi cả cảm xúc của một căn nhà.';
  $story = [
  ...
  $stats = [['10+','Năm kinh nghiệm'],['5000+','Khách hàng'],['100+','Dự án doanh nghiệp'],['40+','Thành viên đội ngũ']];
  $values = [
...
@section('content')
  ...
  <section style="position:relative;min-height:70vh;display:flex;align-items:flex-end;overflow:hidden">
    ...
    <x-container :style="'position:relative;padding-bottom:var(--space-10);padding-top:150px'">
      <x-eyebrow rule color="var(--moss)">Về chúng tôi</x-eyebrow>
      <h1 style="color:#fff;font-size:var(--text-display-xl);line-height:1.05;margin:18px 0 0;max-width:16ch;text-wrap:balance">
        Nghề rèm, thêu — <span style="font-style:italic;color:var(--moss-soft)">chăm chút từng nếp gấp</span>
      </h1>
  ...
  <section style="padding-top:var(--section-y);padding-bottom:var(--section-y);background:var(--cream)">
    <x-container :style="'display:grid;grid-template-columns:1fr 1fr;gap:var(--space-10);align-items:center'">
      <div>
        <x-eyebrow rule>Câu chuyện NẾP</x-eyebrow>
        <h2 style="font-size:var(--text-display-md);margin:16px 0 20px;max-width:16ch">Từ một xưởng nhỏ đến thương hiệu được tin yêu</h2>
        @foreach($story as $p)
          <p style="font-size:var(--text-lg);line-height:1.75;color:var(--text-body);margin-bottom:18px;max-width:50ch">{{ $p }}</p>
        @endforeach
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <img src="https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=700&q=80" alt="" style="width:100%;aspect-ratio:3/4;object-fit:cover;border-radius:var(--radius-lg);box-shadow:var(--shadow-md)">
        <img src="https://images.unsplash.com/photo-1452860606245-08befc0ff44b?w=700&q=80" alt="" style="width:100%;aspect-ratio:3/4;object-fit:cover;border-radius:var(--radius-lg);margin-top:36px;box-shadow:var(--shadow-md)">
      </div>
    </x-container>
  </section>
  ...
  <section style="padding-top:var(--section-y);padding-bottom:var(--section-y);background:var(--paper)">
    <x-container>
      <div style="text-align:center;margin-bottom:var(--space-8)">
        <x-eyebrow rule center>Giá trị cốt lõi</x-eyebrow>
        <h2 style="font-size:var(--text-display-md);margin-top:12px">Điều chúng tôi luôn giữ</h2>
      </div>
  ...
  <section style="padding-top:var(--section-y);padding-bottom:var(--section-y);background:var(--beige)">
    <x-container>
      <div style="margin-bottom:var(--space-8)">
        <x-eyebrow rule>Hành trình</x-eyebrow>
        <h2 style="font-size:var(--text-display-md);margin-top:12px">Những cột mốc đáng nhớ</h2>
      </div>
  ...
  <section style="padding-top:var(--section-y);padding-bottom:var(--section-y);background:var(--paper)">
    <x-container>
      <div style="text-align:center;margin-bottom:var(--space-8)">
        <x-eyebrow rule center>Đội ngũ</x-eyebrow>
        <h2 style="font-size:var(--text-display-md);margin-top:12px">Những con người làm nên NẾP</h2>
      </div>
      <div class="nep-grid-4" style="display:grid;grid-template-columns:repeat(4,1fr);gap:var(--space-5)">
        @foreach($team as $t)
